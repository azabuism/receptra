...
async def force_duplicate_enqueue(
    ...
    # enqueue_reservation_confirmed_call自身が本番実装と全く同じ関数
    # （app.services.outbound_dispatch）。ここでは意図的に2回連続で呼び、
    # idempotency_keyのDB一意制約により2件目がIntegrityError→rollbackで
    # 握りつぶされ、ジョブが1件のままであることを検証する。
...
async def create_forced_failure_job(
    ...
    # call_typeをworkerが認識しない値にすることで、_process_one_job()の
    # 「unsupported_or_missing_data」早期FAILED分岐を確実に踏ませる
    # （Fake providerにすら到達しない＝プロバイダー障害シミュレーションとしても
    # 安全に「発信そのものは一切起きない」ことを保証できる）。
    job = OutboundCallJob(
        shop_id=shop.id,
        reservation_id=reservation_id,
...
async def delete_own_e2e_tenant(
    ...
    # Tenant.users は既存モデルで cascade="all, delete-orphan" が定義済みのため、
    # tenant削除だけでUser行も一緒に削除される（Shopは対応するrelationshipが
    # Tenant側に無いため、上のガードで事前に削除済みであることを保証している）。
