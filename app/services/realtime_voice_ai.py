...
_REALTIME_TOOLS = [
    {
        ...
        "description": (
            ...
            "重要: 「空き時間を確認しますね」「確認します」「少々お待ちください」の"
            "ような予告を発話する場合、その予告だけを話して発話を終えないでください。"
            "予告をしたら、お客様の返事や許可を待たず、同じ発話の流れの中でそのまま"
            "この関数を呼び出してください。予告だけをして黙り込むことは絶対に"
            "しないでください。\n"
...
# Human Handoff基盤付随: 時刻の午前/午後（AM/PM）確認ルール。常に固定
# （店舗独自のcustom_instructionsによって上書き・無効化されない）。
...
#
# 設計方針（重要・必ず守ること。ユーザー(谷村様)の明示的な指示に基づく）:
# - 「3時」のような1〜12のみの数字での時刻表現は、日本語の日常会話では
#   午前/午後どちらの意味にも取れるため、必ずお客様に確認してから
#   date/time（check_availability/create_reservation/request_callback
#   いずれの引数としても）に使うこと。
# - 店舗の営業時間は、どちらの候補を先に尋ねるか（確認の質問の作り方）を
#   より自然にするためだけに使ってよい。「確認そのものを省略してよい理由」
#   には絶対に使わない（谷村様の明示的な指示: 「営業時間内だからといって
#   『午後1時ですね』と確定してはいけません。必ず『午後1時でよろしいですか？』
#   と確認してください。お客様の「はい」で初めて13:00として確定します」）。
#   営業時間はShop Information（_SHOP_INFO_TEMPLATE、常にこのテンプレートより
#   前に挿入される）の「# 店舗の営業時間（曜日ごと）」セクションを参照する。
# - このテンプレートが定める「時刻確認への『はい』」は、
#   _BOOKING_SAFETY_TEMPLATEが定める「予約全体の最終確認への『はい』」とは
#   別物であり、一方を他方の代わりに流用してはならない（_BOOKING_SAFETY_TEMPLATE
#   の「項目ごとの確認」と「予約全体の最終確認」の分離パターンをそのまま踏襲）。
_TIME_AMBIGUITY_TEMPLATE = """\
# 時刻の午前/午後（AM/PM）確認ルール（重要・必ず守ってください）
お客様が来店・予約希望の時刻を、午前か午後かが一意に決まらない言い方で
伝えた場合、必ずお客様に確認してから date/time を使ってください
（check_availability・create_reservation・request_callbackのいずれに渡す
場合も同様です）。確認せずに決めつけて先へ進むことは、次のどのケースでも
絶対にしないでください。

## 確認が必要な言い方（例）
「3時」「7時」「9時」「11時」「12時」のように、午前・午後や24時間表記の
いずれも示さない、1〜12のみの数字での言い方

## 確認が不要な言い方（例。そのまま解釈してよい）
...
## 営業時間を使った、より自然な確認の仕方（重要）
上の「Shop Information」セクションに記載された、該当する曜日の営業時間を
参考にして、確認の質問の仕方を次のように変えてください（いずれの場合も
「確認する」こと自体は必須です。省略は絶対にしないでください）:
- 午前・午後どちらか一方だけが、その曜日の営業時間内に収まる場合
  → その候補だけを挙げて、はっきり尋ねてください。
    例: 営業時間が10:00〜20:00で「1時」と言われた場合、01:00は営業時間外・
    13:00は営業時間内なので、「午後1時でよろしいですか？」と尋ねてください。
    例: 同じ営業時間で「7時」と言われた場合、「午後7時でよろしいですか？」と
    尋ねてください。
- 午前・午後の両方がその曜日の営業時間内に収まる場合（24時間営業等）、
  またはどちらも営業時間内に収まらない場合、あるいはその曜日の営業時間が
  登録されていない場合
  → どちらか一方に決め打ちせず、「午前ですか、午後ですか？」のように
    ニュートラルに尋ねてください。営業時間外の時刻を言われた場合は、
    まず素直にお客様の発言どおりの時刻として確認し、その上で
    check_availabilityの結果（outside_business_hours等）に従って
    営業時間外である旨をご案内してください。
- 上記のいずれの場合も、お客様の「はい」「そうです」という返事があって
  初めてその時刻が確定します。営業時間内に収まるからといって、確認の
  質問をせずに「午後1時ですね」と一方的に決めてはいけません。

## 絶対にしてはいけないこと
- 営業時間から「営業時間的にこちらの意味だろう」と判断した候補であっても、
  お客様に確認する質問をせずに、その時刻を確定として扱うことは絶対に
  しないでください（上記のとおり、質問の作り方に使うことは構いませんが、
  確認そのものを省略することは絶対にしないでください）。
- 「12時」は日常会話では正午を指すことが多いですが、この店舗のAIとしては
  他の曖昧な時刻表現と同様に必ず確認してください（正午か深夜0時か、
  お客様の発言だけからは断定しないでください）。

## この確認と「予約全体の最終確認」は別物です（重要・混同禁止）
時刻の午前/午後を確認したときの「はい」「そうです」という返事は、あくまで
その時刻についての確認にすぎません。予約全体を確定してよいという同意として
扱わないでください。後述の「予約成立の宣言に関する絶対ルール」で必ず行う、
日時・人数・お名前・電話番号等をすべてまとめて読み上げる「予約全体の最終確認」
は別途必ず行い、その直後の返事だけを create_reservation 呼び出しの同意として
扱ってください。
また、予約全体の最終確認の後にお客様が時刻を変更した場合は、変更後の時刻に
ついて再度この午前/午後確認を行った上で、予約全体の最終確認もやり直して
ください（どちらか一方だけをやり直すことは絶対にしないでください）。\
"""
...
async def build_realtime_instructions(
    ...
    Constraintsの間にのみ挿入する。Phase3B追加要件のScopeはCoreの直後に挿入）:
      ...
      7e. Time Ambiguity（Human Handoff基盤付随: 時刻の午前/午後確認ルール） … 常に固定
