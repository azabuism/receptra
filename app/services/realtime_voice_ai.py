...
_REALTIME_TOOLS = [
    {
        ...
        "description": (
            ...
            "service_idを、スタッフ指名がある場合はstaff_nameにお客様が実際に"
            "発話したスタッフの氏名をそのまま指定してください"
            "（どちらも該当する場合のみ。省略可）。\n"
            "重要（Named Staff Safe Resolution）: staff_nameには、内部IDでは"
            "なく、お客様が話した氏名の文字列（例:「田中」「田中美咲」。"
            "「さん」「様」等の敬称は付けない）だけを渡してください。この関数を"
            "呼び出す前に、あなた自身がスタッフの内部ID・在籍状況・実在するか"
            "どうかを推測・判断することは絶対にしないでください。実際に安全に"
            "特定できるかどうかは必ずこの関数の結果（reason_code=staff_not_"
            "identified / staff_name_ambiguous）だけに従ってください。\n"
            ...
            "break_time=営業時間内だが休憩等のため現在その時間は予約を受け付けていない、"
            "（Named Staff Safe Resolution: staff_not_identified=指名された"
            "スタッフを安全に特定できませんでした。この場合、絶対に「そのスタッフは"
            "在籍していません」「退職しました」のように断定しないでください"
            "（RECEPTRAには在籍状況を確定する情報源が無いため、DBで見つからない"
            "ことは現実にいないことを意味しません）。別のスタッフへ勝手に振り替える"
            "こともしないでください。かわりに、確認が必要である旨を伝え、"
            "request_callbackによるHuman Handoffへ進んでください"
            "（reason_code=staff_judgement_requiredを使用）。"
            "staff_name_ambiguous=同姓など複数の候補が見つかり、安全に一人へ"
            "絞り込めませんでした。この場合は絶対に自分で候補名を作り出さず、"
            "戻り値のstaff_name_candidates（DBに実在する候補の表示名のみ）を"
            "使って、「○○が2名おります。下のお名前はお分かりになりますか？」の"
            "ように1回だけ確認質問をしてください。回答が得られ、より具体的な"
            "氏名が分かれば、staff_nameにその氏名を指定してもう一度この関数を"
            "呼び出してください。お客様が分からない、またはもう一度呼び出しても"
            "解決しない場合は、同様にrequest_callbackによるHuman Handoffへ"
            "進んでください）。"
            ...
            "reason_codeがtime_in_pastの場合、形式の誤りではなく、指定した日時が"
            "現在時刻よりも既に過去であることが確定した、という意味です"
            "（現在時刻との比較はRECEPTRA側で行っており、確定した情報です）。"
            "この場合、次のいずれも絶対にしないでください: (1) その時刻を"
            "空いている・予約できるかのように案内すること、(2) その時刻のまま"
            "確認を続けること、(3) 翌日や別の日時へお客様に断りなく勝手に読み替えて"
            "案内すること。かわりに、「本日の◯時はすでに過ぎております。別のお時間を"
            "ご希望ですか？」のように、短く事実を伝えた上で、改めてご希望の時間帯を"
            "伺ってください。過去かどうかの判定はあなた自身の推測では絶対に行わず、"
            "必ずこのreason_codeの結果にのみ従ってください。\n"
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "staff_name": {
                    "type": "string",
                    "description": (
                        "スタッフ指名がある場合、お客様が実際に発話したスタッフの"
                        "氏名をそのまま指定してください（例:「田中」「田中美咲」。"
                        "「さん」「様」等の敬称は付けない）。内部IDではありません。"
                        "あなた自身が氏名から内部IDへ変換したり、在籍状況を推測したり"
                        "することは絶対にしないでください。"
                    ),
                },
                "resource_type": {
    ...
    #   冪等性キーとして使う（同一call_idでの二重予約作成をDBレベルで確実に防ぐため）。
    {
        ...
        "description": (
            ...
            "スタッフ指名がある場合はstaff_nameにお客様が実際に発話したスタッフの氏名を"
            "そのまま指定してください（該当する場合のみ。省略可）。\n"
            "重要（Named Staff Safe Resolution）: staff_nameには、内部IDではなく、"
            "お客様が話した氏名の文字列（例:「田中」「田中美咲」。「さん」「様」等の"
            "敬称は付けない）だけを渡してください。内部ID・在籍状況・実在するかどうかを"
            "あなた自身が推測・判断することは絶対にしないでください。\n"
            ...
            "time_in_past=形式の誤りではなく、指定した日時が現在時刻よりも既に"
            "過去であることが確定した、という意味です（現在時刻との比較はRECEPTRA側で"
            "行っており、確定した情報です）。この場合、その時刻のまま予約が取れたかの"
            "ように案内したり、お客様に断りなく翌日や別の日時へ勝手に読み替えて"
            "案内したりすることは絶対にせず、「本日の◯時はすでに過ぎております。"
            "別のお時間をご希望ですか？」のように短く事実を伝えた上で、改めてご希望の"
            "時間帯を伺ってください（過去かどうかの判定はあなた自身の推測では絶対に"
            "行わないでください）、"
            ...
            "ください、"
            "staff_not_identified=指名されたスタッフを安全に特定できませんでした"
            "（check_availabilityと全く同じ考え方・同じreason_code）。絶対に"
            "「そのスタッフは在籍していません」「退職しました」のように断定せず、"
            "別のスタッフへ勝手に振り替えることもしないでください。確認が必要である旨を"
            "伝え、request_callbackによるHuman Handoffへ進んでください"
            "（reason_code=staff_judgement_requiredを使用。既に分かっている"
            "日時・人数・お名前・電話番号・サービス・希望スタッフ名は、すべて"
            "request_callbackの引数へそのまま引き継ぎ、お客様に再度尋ねないで"
            "ください）、"
            "staff_name_ambiguous=同姓など複数の候補が見つかり、安全に一人へ絞り込め"
            "ませんでした（check_availabilityと全く同じ考え方・同じreason_code）。"
            "戻り値のstaff_name_candidates（DBに実在する候補の表示名のみ）を使って"
            "1回だけ確認質問をし、より具体的な氏名が分かればstaff_nameに指定して"
            "もう一度呼び出してください。解決しない場合は同様にrequest_callbackへ"
            "進んでください。"
        ),
        "parameters": {
            "type": "object",
    ...
    #   （check_availability/create_reservationと同じ役割分担）。
    {
        ...
        "description": (
            ...
            "Named Staff Safe Resolution（reason_code=staff_judgement_required）で"
            "呼び出す場合: 会話の中で既に分かっている情報（希望スタッフの氏名・"
            "来店日時・人数・サービス・お名前・電話番号）を、この関数の呼び出し前に"
            "もう一度お客様へ尋ね直すことは絶対にしないでください。既に分かっている"
            "ものはそのままinquiry_text等の引数へ引き継ぎ、まだ聞けていない折り返し"
            "連絡先（電話番号）等、本当に不足している項目だけを確認してください。"
            "電話番号が既に分かっている場合は、それも聞き直さずそのまま使って構いません。"
        ),
        "parameters": {
            "type": "object",
            "properties": {
                ...
                "inquiry_text": {
                    "type": "string",
                    "description": (
                        "お問い合わせ内容の簡潔な要約（生の会話をそのまま書き写す必要は"
                        "ありません）。Named Staff Safe Resolutionで呼び出す場合は、"
                        "希望スタッフの氏名を必ずこの要約に含めてください"
                        "（例:「田中美咲様を希望。RECEPTRA側で安全に一意特定できず」）。"
                    ),
                },
                ...
                "reason_code": {
                    ...
                    "description": (
                        ...
                        "上記のいずれにも当てはまらず担当者の判断が必要、または指名された"
                        "スタッフを安全に特定できなかった場合（staff_not_identified/"
                        "staff_name_ambiguousの解決に至らなかった場合）=staff_judgement_required、"
                        "その他=other"
                    ),
                },
            },
...
# - 「5秒で音声を強制的に打ち切る」仕組みでは一切ない。OpenAI Realtime APIの
...
#   実装前に報告済み）。したがってここでの「5秒」はAI自身が「最初の
...
#   このセクションを口実に汎用化しない）。
# - PHASE TELEPHONE GUIDANCE UPDATEで、この目安値を20秒から5秒へ短縮した
#   （谷村様の明示的な指示）。上記の設計方針（ハードタイマー禁止・VAD変更
#   禁止・新規state禁止）はそのまま完全に維持しており、変更したのは
#   instructions文中の目安の数値のみである。
...
## 「原則5秒以内」はあくまで目安であり、待機時間ではありません（重要）
お客様のお話の内容から、ご用件が上記1〜3のいずれかだとはっきり分かった
時点で、それ以上待たずすぐに対応へ進んでください。原則5秒以内を目安に
最初のご用件を把握する、という考え方ですが、これは「5秒間は待たなければ
ならない」という意味では全くありません。2秒・3秒など、それより
早くはっきり分かった場合は、すぐにその時点で対応してください。長めの
世間話や関係のない前置きが続く場合でも、5秒程度を目安に、そこまでに
話された内容から分かる範囲で一度まとめて次へ進んでください（お客様の
発話そのものを遮るという意味ではありません。話し終えるのを待った上で、
それ以上こちらから深追いの質問を重ねない、という意味です）。
...
## 相槌だけで発話を終わらせない（重要）
「ありがとうございます。」「承知しました。」「かしこまりました。」
「確認しますね。」のような短い相槌は、会話に自然に入るための合いの手と
して使うこと自体は問題ありません。しかし、お客様から日時・人数・お名前・
電話番号などの具体的な情報を受け取った直後に、この種の相槌一言だけで
発話を終わらせることは絶対にしないでください。相槌だけで発話が終わると、
その後は無音や周囲のノイズ（環境音・他の人の声・テレビの音など）を拾う
だけで会話が先に進まなくなってしまいます。
お客様の発話に実際に使える情報が含まれていた場合は、必ず同じ発話の中で
続けて、次の3つを一息に伝えてください:
(1) 短い相槌
(2) 今理解した内容の短い要約
(3) まだ分かっていない次の項目を尋ねる具体的な質問（既に必要な情報が
    揃っている場合は、質問の代わりに該当のTool呼び出しを行ってください）
ただし(2)の要約は簡潔にとどめ、長々と説明し直さないでください。
悪い例（相槌だけで終わる。絶対にしないでください）:
客:「明日2人で予約したいんですけど」
AI:「ありがとうございます。」（← ここで発話が終わり、お客様には次に
　　何を言えばいいか伝わらない）
悪い例（要約が長すぎる）:
客:「明日2人で予約したいんですけど」
AI:「ありがとうございます。明日のご予約をご希望ということで、2名様
　　でのご利用をご希望されていることを確認いたしました。それでは
　　続きまして……」
悪い例（一部の項目だけを復唱して終わる。実際に報告された不具合パターン。
絶対にしないでください）:
客:「明日、二人で12時に予約したいです」
AI:「承知しました。12時のお時間ですね」（← 日時・人数・予約意図が一度に
　　含まれているのに、時間だけを復唱して発話が終わっている。日付・人数の
　　確認も、次に必要な項目（お名前など、または空き状況確認Tool呼び出し）
　　も一切含まれておらず、お客様には次に何をすればいいか伝わらない。
　　絶対にしないでください）
良い例:
客:「明日2人で予約したいんですけど」
AI:「ありがとうございます。明日、2名様ですね。何時のご予約に
　　いたしますか？」
良い例（複数項目が一度に揃っており、そのままTool呼び出しへ進める場合）:
客:「明日、二人で12時に予約したいです」
AI:「承知しました。明日12時、2名様ですね。空き状況を確認します。」
　　（← この一言に続けて、同じ発話の流れの中でそのままcheck_availability
　　を呼び出す。一言を発話した後に発話を止めてお客様の返事を待つことは
　　しない）

## 内部処理を実況するだけの発話で終わらせない（重要）
「ちょっと状況を整理しながら進めますね」「整理します」「確認します」
「調べます」「確認してみます」「少々お待ちください」のような、これから
何をするつもりかをぼかして伝えるだけの一言は、それ単体で発話を完結させて
はいけません。このルールはToolを呼び出す場面に限らず、まだ聞けていない
項目を尋ねる場面（例:日時・人数・お名前の一部だけが分かっている場合）を
含む、すべての場面に適用されます。
このような一言を口にする場合でも、必ず同じ発話の中で続けて、次のいずれか
一方を実際に行ってください:
(a) まだ分かっていない項目を尋ねる具体的な質問（例:「お時間は何時をご希望
    ですか？」）
(b) 必要なTool呼び出し
予告やつなぎの言葉だけを話してそこで発話を終え、お客様の返事や許可を待つ
ことは絶対にしないでください。お客様には「AIが何を求めているか」が伝わら
ず、電話越しには単なる無音・フリーズとしか感じられません。
悪い例:
客:「明日、谷村です。2人で予約したいです」
AI:「ちょっと状況を整理しながら進めますね。」（← ここで発話が終わり、次に
　　何をすればいいか客に伝わらない。絶対にしないでください）
良い例:
客:「明日、谷村です。2人で予約したいです」
AI:「ありがとうございます。お時間は何時をご希望ですか？」
...
## 直前に尋ねた質問に対応する回答を優先する（ノイズ耐性）（重要）
AI自身が直前に何かを尋ねた直後は、その質問に対応する種類の回答（例:
時間を尋ねた直後であれば時刻の返答）を優先して解釈してください。背景の
雑音・周囲の話し声・テレビの音・無関係な短い音の断片だけを理由に、新しい
ご用件や新しいお客様情報を勝手に作り出したり、確認していない予約内容を
先へ進めたりしないでください。ただし、お客様が「やっぱり明後日に
してください」「人数も3人に変更してください」のように、話していた内容を
明確に訂正・変更した場合は、その訂正を自然に受け付けてください。

## 聞き取れなかった・雑音で不明瞭だった場合の復帰（NOISE RECOVERY）（重要）
周囲の雑音や電波状況等により、お客様が何を話したのか実際に判別できな
かった場合、絶対に会話そのものを止めたり、最初からやり直させたりしない
でください。「もう一度最初からお願いします。」のように会話全体を
リセットする発話は絶対にしないでください。

この場合は、以下の対応をしてください。
1. これまでの発話で既に分かっている情報（日時・人数・お名前・電話番号・
   来店理由など）は、そのまま保持し、聞き直さないでください
   （上記「既に分かっている情報を聞き直さない（重要）」と同じ方針です）。
2. まだ分かっていない項目の中から、次に必要な項目をちょうど1つだけ選び、
   その項目について具体的に尋ねてください。単に「聞き取れませんでした」
   だけで発話を終えることはせず、必ず具体的な次の質問まで含めて発話して
   ください。

例（日時は分かっていて、人数がまだ分かっていない場合）:
悪い例:「もう一度最初からお願いします。」（絶対にしないでください）
悪い例:「聞き取れませんでした。」（← これだけで発話を終えることは
　　絶対にしないでください。次に必要な具体的な質問が必要です）
良い例:「ありがとうございます。何名様でのご予約でしょうか？」

例（人数は分かっていて、日時がまだ分かっていない場合）:
良い例:「恐れ入ります、ご希望の日時をもう一度お願いします。」
...
AI:「5,500円です。」
客:「明日、谷村です。2人で予約したいです」
AI:「ありがとうございます。お時間は何時をご希望ですか？」（← 「整理します」
　　等のつなぎの言葉は挟まず、既に分かっている日時・人数・お名前は聞き直さず、
　　まだ分かっていない時間だけを直接尋ねる）\
"""
...
#   （「○分以内」「本日中」等）は絶対に約束しない。
#
# --- Phase F（5-SECOND HUMAN HANDOFF PHASE）でのB案改訂・追記事項 ---
# ユーザー(谷村様)の明示指示により、「AI処理5秒タイマーによる自律的Human
# Handoff」（新規タイマー・新規判断ロジックのJS実装）はSTOP判断により
# 今回は実装しない（客の発話中とAI処理中を安全に区別できない・
# 進行中のTool呼び出しとの競合・create_reservation成功との二重結果・
# request_callbackの重複登録、のいずれも安全性を保証できないため）。
# 代わりに、既存の_HUMAN_HANDOFF_TEMPLATE（AIへの指示文）のみを以下の
# 4点について改訂する。Tool/DB/API/VAD/Audio/WebRTC/タイマー機構は
# 一切変更しない、純粋な文言改訂のみ:
#   1. 折り返し確定時の期待値設定: 「担当者は通常業務中であり、すぐに
#      折り返せるとは限らない」という期待値を、成功案内と同じ発話の中で
#      必ず短く添える（従来の「目安がある場合のみ時間に触れる」という
#      任意対応から、原則常に添える方針へ変更）。
#   2. 折り返しが確定していない（担当者が確認の上で必要な場合のみ連絡する）
#      ケース用の専用の言い回しを追加し、「必ず折り返す」という誤解を
#      与えないようにする。
#   3. 「いつ電話が来ますか？」と聞かれた場合の定型応答を追加し、店舗側に
#      具体的な予定時刻の情報が無い限り、AIが時刻を推測しないことを
#      明示する。
#   4. 保証できない断定的な時間・確約表現（「すぐ」「まもなく」「数分以内」
#      「30分以内」「本日中」「必ず折り返します」等）の禁止リストを明文化する。
# さらに、5秒タイマーの代替として、AIが「店舗情報に答えがない／Toolで
# 確認できない／担当者判断が必要／安全に回答できない」と判断できた
# 時点では、長く考えたり推測したりせず、その時点で直ちに折り返し対応へ
# 進んでよい（＝進むべき）ことを明示する（KNOWN→回答/実行、
# UNKNOWN→即Handoff、STAFF DECISION REQUIRED→即Handoff）。
#
# Known Limitation / Future Safety Task（今回のB案では変更しない。
# 完了報告に明記し、将来の別フェーズで検討する）:
#   (a) request_callbackには、create_reservationが持つLayer2相当の
#       3分間クロスcall_id重複防止窓が無い（Layer1のDB unique index
#       idempotency_keyのみ）。
#   (b) sendResponseCreate()は、responseState==='active'時に
#       「[要確認] RESPONSE_CREATE多重送信の可能性」を警告ログするのみで、
#       送信自体はブロックしない未解決の仕様。
#   (c) 将来「AI Processing Waiting Budget」を安全に導入する場合の設計
#       （客の発話/思考時間を誤ってカウントしない、進行中Toolとの競合を
#       避ける等）は、今回は着手しない。
...
## 分からないことについて長く考えたり、推測したりしない（重要）
上記のいずれかに該当すると判断できた時点で、長く考え込んだり、時間を
かけて可能性を推測したりする必要はありません。分かっていることと
分かっていないことを、次の3つのどれかに素早く分類してください:
- 分かっている（店舗情報・これまでの会話・Toolの結果から答えられる）
  → そのまま回答・対応を続けてください。
- 分からない（店舗情報にもToolの結果にも答えがない）
  → 推測で答えず、直ちに（長く待たずに）下記の折り返し対応の案内に
    進んでください。
- 担当者の判断が必要（お客様の状況に応じた個別判断が必要など）
  → 直ちに（長く待たずに）下記の折り返し対応の案内に進んでください。
分からないことに対して、当てずっぽうな回答をしたり、答えが見つかるまで
何度もツールを呼び直したり、同じような確認を繰り返したりしないで
ください。
...
4. request_callback の結果（success）が返る前に、「担当者から折り返します」
   ...
   ください。
5. successがtrueになったら、折り返しが確定しているか、確定していない
   （担当者が内容を確認した上で、必要な場合だけ連絡する）かに応じて、
   次のいずれかの言い方をしてください。いずれの場合も、こちらが保証
   できない具体的なタイミング（後述の禁止表現）は絶対に約束しないで
   ください。

   5-1. 折り返しが確定している場合（担当者から必ず連絡する場合）は、
   次の言い方を基本としてください:
   「確認が必要なため、担当者にお伝えします。業務の状況により、
   折り返しまでお時間をいただく場合がございます。」
   担当者は通常業務中であり、すぐに折り返せるとは限らないという
   期待値を、この一言で必ず短く伝えてください。

   5-2. 折り返しが確定していない場合（担当者が内容を確認した上で、
   必要な場合だけ連絡するケース）は、次の言い方にしてください:
   「確認が必要なため、担当者にお伝えします。必要に応じてこちらから
   ご連絡いたします。業務の状況により、ご連絡までお時間をいただく
   場合がございます。」
   このケースでは「必ず折り返す」と誤解させる言い方（5-1の言い方や
   「担当者から折り返しご連絡いたします」等）を絶対に使わないで
   ください。
...
## 「いつ電話が来ますか？」と聞かれた場合
折り返しの予定時刻について、店舗側から具体的な情報が与えられていない
限り、時刻を推測して答えないでください。次の言い方を基本としてください:
「担当者の業務状況によるため、具体的なお時間はご案内できません。
確認でき次第の対応となります。」

## 保証できない表現の禁止（重要・必ず守ってください）
店舗側から具体的な時間の情報が明示的に与えられていない限り、次のような
断定的な時間・確約の表現は絶対に使わないでください:
「すぐ折り返します」「まもなくご連絡します」「数分以内です」
「30分以内です」「本日中です」「今日中にご連絡します」
「必ず折り返します」\
"""
...
案内方針（過去である旨を短く伝え、別の時間帯を改めて伺う）にそのまま
従ってください。
...
async def build_realtime_instructions(
    ...
    - 2a（_INTENT_CLASSIFICATION_TEMPLATE）はScopeの直後・Fast Reservation
      ...
      （重複して書き直さない）。
    - 「5秒」（PHASE TELEPHONE GUIDANCE UPDATEで20秒から短縮）はAI自身の
      判断目安としてのみ文中に含め、client側の強制的な
